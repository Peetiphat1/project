...
interface Shoe {
  ...
  maxDistance: number
  treadWear: number          // 0–100 percentage
  status: GearStatus
  addedDate: string
  lastUsed: string
  bestFor: string
...
function WearBar({ pct, status }: { pct: number; status: GearStatus }) {
  ...
  const trackLabel = `Tread wear: ${pct}%`
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-1.5">
        ...
        <span
          className={`font-mono text-xs font-bold ${status === 'Retiring Soon' ? 'text-red-500' : 'text-slate-700'}`}
        >
          {pct}%
        </span>
      </div>
      ...
      >
        <div
          className={`h-full ${barColor} rounded-sm relative overflow-hidden transition-all duration-700`}
          style={{ width: `${pct}%` }}
        >
          {/* Tread texture grooves */}
          {[...Array(10)].map((_, i) => (
            <div
              key={i}
...
/** Shoe silhouette placeholder (no external image needed) */
function ShoeIllustration({ initial, color }: { initial: string; color: string }) {
  return (
    <div
      className="relative w-full h-36 rounded-sm overflow-hidden flex items-center justify-center"
    ...
    >
      ...
      <svg viewBox="0 0 200 100" className="w-48 h-24 opacity-80" aria-hidden="true">
        ...
        {/* Upper */}
        <path
          d="M 20 85 Q 18 55 40 50 L 110 42 Q 140 38 160 48 Q 180 55 185 70 L 188 85 Z"
          fill={color}
          opacity="0.85"
        />
        {/* Midsole */}
        <path
          d="M 18 85 Q 18 92 30 93 L 180 93 Q 192 92 188 85 Z"
          fill={`${color}60`}
        />
        {/* Laces */}
        {[0, 1, 2, 3].map((i) => (
        ...
        {/* Brand initial */}
        <text x="155" y="78" fontSize="20" fontWeight="bold" fill="white" opacity="0.5"
      ...
      <div className="absolute top-3 right-3 w-9 h-9">
        <svg viewBox="0 0 36 36" aria-hidden="true">
          ...
          <circle cx="18" cy="18" r="15" fill="none" stroke={color} strokeWidth="3"
            strokeDasharray={`${(94.2 * 0.56).toFixed(0)} 94.2`}
            strokeLinecap="round"
...
/** Main active gear card */
function GearCard({ shoe }: { shoe: Shoe }) {
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-[#F5F5F3] rounded-sm p-2.5 flex flex-col gap-0.5">
            ...
            <span
              className={`font-mono text-lg font-bold tabular-nums ${shoe.status === 'Retiring Soon' ? 'text-red-600' : 'text-slate-900'}`}
            >
              {remaining}
              <span className="text-xs text-slate-400 ml-0.5">km</span>
            </span>
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="font-mono text-xs text-slate-600 font-bold">
              {shoe.distanceLogged} / {shoe.maxDistance} km
            </span>
          </div>
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`Usage: ${pctUsed}% of max distance`}
          >
            <div
              className={`h-full rounded-full transition-all duration-700 ${shoe.status === 'Retiring Soon' ? 'bg-red-500' : 'bg-slate-400'}`}
              style={{ width: `${pctUsed}%` }}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 mt-auto pt-1">
          {shoe.status === 'Default' ? (
            <button
              id={`btn-default-${shoe.id}`}
              disabled
              className="w-full py-2 text-xs font-bold tracking-widest bg-orange-50 text-orange-600 border border-orange-200 rounded-sm uppercase cursor-default flex items-center justify-center gap-1.5"
              aria-label={`${shoe.model} is the default shoe`}
            >
            ...
            <button
              ...
              aria-label={`Set ${shoe.model} as the default shoe`}
            >
          ...
          <button
            ...
            className={[
              'w-full py-2 text-xs font-bold tracking-widest rounded-sm border uppercase transition-all flex items-center justify-center gap-1.5',
              shoe.status === 'Retiring Soon'
                ? 'bg-red-600 text-white border-red-600 hover:bg-red-700'
                : 'text-slate-500 border-slate-200 hover:border-red-400 hover:text-red-500',
            ].join(' ')}
...
/** Retired shoe horizontal card */
function RetiredCard({ shoe }: { shoe: RetiredShoe }) {
  return (
    <article
      id={`retired-card-${shoe.id}`}
      className="bg-white/60 border border-slate-200/70 rounded-sm p-4 flex items-center gap-4 opacity-70 hover:opacity-90 transition-opacity"
      aria-label={`Retired: ${shoe.brand} ${shoe.model}`}
    >
      {/* Silhouette placeholder */}
      <div
        className="w-20 h-14 bg-slate-100 rounded-sm flex items-center justify-center shrink-0 overflow-hidden"
        aria-hidden="true"
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-1.5">
          <span className="flex items-center gap-1 text-[10px] font-mono text-slate-400">
            ...
            {shoe.totalDistance} km lifetime
          </span>
          <span className="flex items-center gap-1 text-[10px] font-mono text-slate-400">
            <Calendar className="w-3 h-3" aria-hidden="true" />
            {shoe.seasons}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-[11px] font-bold tracking-widest text-slate-400 uppercase">
            {activeShoes.length} Active Pairs ·{' '}
            <span className="font-mono">{totalKm.toLocaleString()}</span> km Logged This Year
          </p>
        </div>
      ...
      <section id="active-gear-section" aria-labelledby="active-gear-heading">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {activeShoes.map((shoe) => (
            <GearCard key={shoe.id} shoe={shoe} />
          ))}
        </div>
      </section>
      ...
      {activeShoes.some((s) => s.status === 'Retiring Soon') && (
        <aside
          id="retiring-soon-banner"
          className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-sm p-4"
          role="alert"
        >
          <AlertTriangle className="w-5 h-5 text-red-600 shrink-0" aria-hidden="true" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-red-700">
              {activeShoes.filter((s) => s.status === 'Retiring Soon').map((s) => s.model).join(', ')}{' '}
              — approaching end of lifespan
            </p>
            <p className="text-xs text-red-500 mt-0.5">
              Replace within the next few runs to avoid injury risk from degraded cushioning.
            </p>
          </div>
          <button
            id="shop-replacement-btn"
            className="shrink-0 px-3 py-1.5 text-xs font-bold tracking-widest bg-red-600 text-white rounded-sm hover:bg-red-700 uppercase transition-colors"
          >
            Shop Replacement
          </button>
        </aside>
      ...
      {/* ── Retired Vault ────────────────────────────────────── */}
      <section
        id="retired-gear-section"
        aria-labelledby="retired-vault-heading"
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <h2
              id="retired-vault-heading"
              className="font-extrabold text-xl tracking-tight text-slate-400"
            >
              RETIRED PERFORMANCE VAULT
            </h2>
            <p className="text-[11px] text-slate-400 mt-1 tracking-wide">
              {retiredShoes.length} pairs honourably discharged ·{' '}
              <span className="font-mono">
                {retiredShoes.reduce((s, sh) => s + sh.totalDistance, 0).toLocaleString()}
              </span>{' '}
              km served
            </p>
          </div>
        ...
        {/* Empty-state CTA */}
        <button
          id="view-full-archive-btn"
          className="mt-4 w-full py-3 text-xs font-bold tracking-widest text-slate-400 border border-dashed border-slate-200 rounded-sm hover:border-slate-300 hover:text-slate-600 uppercase transition-all"
        >
          View Full Archive →
        </button>
      </section>
